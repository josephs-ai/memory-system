...
# Score used when a value is absent from the maps below. Any real vocabulary
# value falling through to this is a bug: the extractor emits
# DurabilityClass.durable_eligible, but the map only knew "durable", so the most
# valuable items silently scored 0.5 and importance was pinned at 0.775 against
# a 0.85 promotion gate -- auto-promotion was mathematically impossible while
# extraction kept filling the queue. test_vocabulary_coverage.py asserts these
# stay in step with the enums.
UNKNOWN_SCORE = 0.5
